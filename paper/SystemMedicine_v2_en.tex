...
\hypersetup{
    ...
    pdfsubject={System-medicine concept paper on pathway-centric exclusion logic for differential-diagnosis research},
    pdfkeywords={knowledge graph, differential diagnosis, functional pathways, exclusion reasoning, system medicine, Reactome, Gene Ontology},
}
...
\begin{quote}
\emph{If a biological functional pathway is sufficiently supported as intact, then genes whose pathogenic effect would be expected to disrupt that pathway can be deprioritized as primary explanations for the current presentation.}
\end{quote}

This transforms positive evidence (apparently preserved pathway function) into negative diagnostic constraints (lower-priority candidate genes), potentially reducing the candidate space before more detailed differential-diagnostic review begins.
...
The proposed system addresses this gap by treating functional pathways as \emph{primary anchor entities} in a knowledge graph, with all other biomedical concepts---genes, anatomical locations, cell types, laboratory markers, and diagnoses---attached as secondary annotations. This structural choice makes pathway-status reasoning explicit and computationally tractable while preserving a biologically interpretable intermediate layer.
...
\begin{enumerate}
    ...
    \item A \textbf{formal exclusion framework} with both binary and probabilistic variants, enabling systematic prioritization and deprioritization of candidate genes based on pathway-integrity evidence.
...
\label{def:binary}
If $s(p_i) = 1$, then $\forall\, g \in \mathcal{E}(p_i)$: gene~$g$ is flagged for \emph{exclusion} from the current primary-cause candidate set, provided $g$ is essential only in pathways that are all assessed as intact:
\begin{equation}
...
\begin{tikzpicture}[
    ...
    \node[entity, fill=purple!10, right=2cm of measurement] (signature) {Measurement\\Signature};
    \node[entity, fill=teal!20, above=3.5cm of pathway] (datasource) {Data\\Source};
...
\begin{itemize}
    \item \textbf{FunctionalPathway}: name, description, primary output, temporal class (acute or chronic), status (intact, disrupted, or unknown), and external ID (Reactome/GO).
...
The prototype uses \textbf{SQLite} as its backend, with relational and association tables emulating graph structure (Figure~\ref{fig:schema_sql}). The system comprises four main modules:
...
KEGG \citep{Kanehisa2023} & Pathway maps & REST API & Planned \\
OMIM \citep{Amberger2019} / HPO \citep{Robinson2008} & Disease--gene links & --- & Planned \\
\bottomrule
\end{tabularx}
\end{table}
...
Confidence in exclusion depends on the number of intact pathways: a gene excluded via multiple independent intact pathways has stronger support than one excluded via a single pathway.
...
Genes are categorized by exclusion confidence: high ($\geq 0.8$), moderate ($0.3$--$0.8$), or suspect ($< 0.3$). This tripartite classification is intended to support ranked review rather than to produce an automatic clinical decision.
...
% ================================================================
\section{Hemolysis and Immunodeficiency Demonstration}
\label{sec:demo}
...
``Essential gene'' is defined operationally as a gene annotated in Reactome or curated sources as \emph{required} for at least one reaction in the pathway. This definition is intentionally conservative and should be refined using curated essentiality databases (e.g., DepMap, CRISPR essentiality screens) and disease-specific expert review.
...
Recent work combines large language models with biomedical knowledge graphs. ESCARGOT \citep{Matsumoto2025} integrates LLMs with a dynamic ``Graph of Thoughts'' architecture and biomedical knowledge graphs for multihop reasoning tasks. While ESCARGOT reports effective graph-based reasoning in biomedical contexts, it operates as a general-purpose reasoning agent without explicit exclusion logic. The proposed system shares the emphasis on knowledge graph-guided reasoning but differs in its organizing principle: functional pathways as explicit diagnostic constraints rather than general-purpose graph traversal.
...
\subsection{Evaluation-Gate Ledger}

Evaluation cannot be reduced to a single accuracy score. Demonstration data, synthetic stress tests, real patient benchmarks, temporal validity, provenance, and external comparators answer different questions. Table~\ref{tab:evaluation-gates} separates these layers so that a positive result in one layer is not mistaken for validation in another.

\begin{table}[H]
\centering
\caption{Evaluation gates for pathway-certificate validation.}
\label{tab:evaluation-gates}
\scriptsize
\begin{tabularx}{\textwidth}{@{}YYYY@{}}
\toprule
\textbf{Gate} & \textbf{Minimum data object} & \textbf{Pass / hold rule} & \textbf{Overclaim prevented} \\
\midrule
...
Real-case benchmark & 20--50 confirmed rare-disease cases with pre-diagnosis laboratory profiles & CRR $\geq 0.30$ with false-exclusion rate $=0$ & No autonomous diagnostic claim \\
...
Comparator suite & Same case set evaluated with HPO/Phenomizer and reproducible rare-disease KG or embedding baselines & Added value beyond HPO-only filtering and current baseline systems is demonstrable & No isolated benchmark claim \\
\bottomrule
\end{tabularx}
\end{table}
...
The candidate reduction rate (CRR, Equation~\eqref{eq:crr}) is used here as the primary success criterion. A CRR $> 0.30$ supports the core hypothesis; a CRR $< 0.10$ would indicate insufficient diagnostic value.
