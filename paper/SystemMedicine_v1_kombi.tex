...
\hypersetup{
    ...
    pdftitle={Functional Pathway-Centric Medical Knowledge Graph with Exclusion Logic (EN/DE Combined)},
...
\title{%
    ...
    \large A System-Medicine Architecture for Differential Diagnosis Support\\[0.3em]
    \normalsize (Mit deutscher \"Ubersetzung im Anhang / With German translation in appendix)
}
\author{
    Lukas Geiger\\
...
\date{March 2026 --- Concept Paper v0.2 (Combined EN/DE)}
...
% ================================================================
%
%   APPENDIX: DEUTSCHE UEBERSETZUNG
%
% ================================================================
% ================================================================
...
\label{de:sec:architecture}

\subsection*{Datenmodell}
\label{de:sec:datamodel}

Die zentrale Entitaet ist der \textbf{FunctionalPathway}-Knoten, der einen diskreten biologischen Prozess mit beobachtbaren Ein- und Ausgaben repraesentiert. Abbildung~\ref{de:fig:schema} zeigt das vollstaendige Datenmodell.
...
Kantentypen tragen semantische Bezeichnungen (Tabelle~\ref{de:tab:edges}) und bei Gen-Pathway-Kanten ein \texttt{is\_essential}-Flag, das modulatorische von essentiellen Genbeitraegen unterscheidet.
...
\label{de:sec:implementation}

Der Prototyp verwendet \textbf{SQLite} als Backend, wobei relationale Tabellen eine Graphstruktur ueber Assoziationstabellen simulieren (Abbildung~\ref{de:fig:schema_sql}). Das System umfasst vier Hauptmodule:

\begin{enumerate}
    \item \textbf{Aufnahmemodul}: Laedt sechs oeffentliche Datenquellen herunter und parst sie (Abschnitt~\ref{de:sec:datasources}), mit einem manifestbasierten Tracking-System zur Vermeidung redundanter Downloads.
...
OMIM / HPO \citep{Robinson2008} & Krankheit-Gen-Links & --- & Geplant \\
\bottomrule
\end{tabular}
\end{table}
...
Die probabilistische Engine (Definition~\ref{de:def:probabilistic}) erweitert das binaere Modell durch Zuweisung kontinuierlicher Konfidenzwerte. Drei Faktoren tragen bei:
...
Fuenf Pathways werden modelliert (Tabelle~\ref{de:tab:pathways}), die zwei klinische Domaenen (Haematologie und Immunologie) mit unterschiedlichen zeitlichen Charakteristiken umfassen.

\begin{table}[H]
\centering
\caption{Funktionelle Pathways im Demonstrationsszenario.}
\label{de:tab:pathways}
\begin{tabular}{lll}
\toprule
...
Elf Gene sind mit diesen Pathways verknuepft, mit expliziten Essentialitaetsannotationen (Tabelle~\ref{de:tab:genes}). Das Essentialitaetsflag ist das entscheidende Attribut, das die Ausschlusslogik ermoeglicht: Nur essentielle Gene koennen ausgeschlossen werden, wenn ihr Pathway intakt ist.
...
Der Quellcode des Prototyps, einschliesslich Datenbankschema, Aufnahmepipelines, Ausschluss-Engines und GUI-Anwendung, ist als Open-Source-Software verfuegbar. Das System integriert ausschliesslich oeffentliche Datenquellen; es werden keine Patientendaten verwendet oder benoetigt.
